...
if st.session_state.mod is None:
    if not is_app_mode:
        ...
        with col1:
            if st.button("📘 Efendi Örnek Proje", use_container_width=True):
                st.session_state.mod = "efendi"
                st.rerun()
        with col2:
